...
- Метод doStep должен учитывать скорость бегуна. Если скорость бегуна 2 шага в секунду, метод должен работать пол секунды;
 если скорость бегуна 4 шага в секунду, метод должен работать четверть секунды.

 - Вывод программы должен отображать сколько шагов сделали Иванов и Петров за 2 секунды.


 как реализовать метод  private void doStep() throws InterruptedException {
            stepNumber++;
я уже по всякому пробовал, помогите разобраться плиииз
 */
public class SleepTask2 {
 ...
 */
        private void doStep() throws InterruptedException {
            stepNumber++;
            switch (owner.getSpeed()){
                case 2: Thread.sleep(1000/2);
                case 4: Thread.sleep(1000/4);
                default: Thread.sleep(1000/owner.getSpeed());
            }
            System.out.println(owner.getName() + " делает шаг №" + stepNumber + "!");
//            if(owner.getSpeed()==2) Thread.sleep(1000/owner.getSpeed());
//            else if(owner.getSpeed()==4) Thread.sleep(2000/owner.getSpeed());
//            System.out.println(owner.getName() + " делает шаг №" + stepNumber + "!");
